...
def test_primary_and_tracker_have_equivalent_surviving_official_findings() -> None:
    ...
    assert [
        ...
        ("arch:linux", "ASA-2026-0001"),
        ("arch:openssl", "ASA-2026-0002"),
    ]
...
def test_correlation_discards_equal_or_newer_fixed_versions_and_keeps_no_fix() -> None:
    ...
    advisories = parse_arch_audit(
        b'[{"name":"ASA-1","packages":["older"],"status":"Fixed","type":"security","severity":"High","fixed":"1:2.0-1","issues":["CVE-2026-0004"]},{"name":"ASA-2","packages":["equal"],"status":"Fixed","type":"security","severity":"Medium","fixed":"2.0-1","issues":[]},{"name":"ASA-3","packages":["nofix"],"status":"Unknown","type":"security","severity":"Unknown","fixed":null,"issues":[]}]'
    )
    run, calls = _vercmp(("-1", "0"))
    ...
    assert calls == [("2.0-1", "1:2.0-1"), ("2.0-1", "2.0-1")]
...
@pytest.mark.parametrize(
    ...
    (
        b"{}",
        b'[{"name":"ASA","packages":[],"status":"Not affected","type":"security","severity":"Low","fixed":null,"issues":[]}]',
        b'[{"name":"ASA","packages":["pkg"],"status":"Vulnerable","type":"security","severity":"High","fixed":null,"issues":[true]}]',
    ),
)
