...
readonly -a png_options=(-strip -set date:create '1970-01-01T00:00:00Z' -set date:modify '1970-01-01T00:00:00Z' -define png:exclude-chunk=date,time -define png:compression-level=9)
...
label_capture() {
  ...
  magick "${source}" -gravity north -background '#121820' -fill '#f5f7fb' -font DejaVu-Sans -pointsize 18 \
    ...
    label:"${title}" -append "${png_options[@]}" "PNG32:${target}"
}
...
magick -size 1360x84 xc:'#0c1119' -gravity center -fill '#ffffff' -font DejaVu-Sans -pointsize 24 \
  -annotate 0 "OPATCHY PREVIEW - ${notice}" "${png_options[@]}" "${stage}/header.png"
magick "${screenshot_root}/fixture-clear.png" "${screenshot_root}/fixture-updates.png" +append "${png_options[@]}" "${stage}/top.png"
magick "${screenshot_root}/fixture-security-stale.png" "${screenshot_root}/fixture-transparent-stale.png" +append "${png_options[@]}" "${stage}/bottom.png"
magick "${stage}/header.png" "${stage}/top.png" "${stage}/bottom.png" -append "${png_options[@]}" "PNG32:${root}/preview.png"
