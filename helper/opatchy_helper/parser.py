...
def parse_response(value: JsonValue) -> Response:
    ...
    generation_id = GenerationId(
        _identifier(reader.field("generationId"), "response.generationId")
    )
    kind = _enum(ResponseKind, reader.field("kind"), "response.kind")
    if kind is ResponseKind.SNAPSHOT:
        _reject_field(reader, "error")
        return _validated(
            SnapshotResponse(
                generated_at, generation_id, _snapshot(reader.field("payload")), version
            )
        )
    if kind is ResponseKind.INVENTORY:
        _reject_field(reader, "error")
        return _validated(
            InventoryResponse(
                generated_at,
                generation_id,
                _inventory(reader.field("payload")),
                version,
            )
        )
    if kind is ResponseKind.STAR_RESULT:
        _reject_field(reader, "error")
        return _validated(
            StarResultResponse(
                generated_at,
                generation_id,
                _star_result(reader.field("payload")),
                version,
            )
        )
    ...
    return _validated(
        ErrorResponse(
            generated_at,
            generation_id,
            _error(reader.field("error"), "response.error"),
            version,
        )
    )
...
def _snapshot(value: JsonValue) -> SnapshotPayload:
    ...
    sources = tuple(
        ...
        for entry in _list(reader.field("sources"), "snapshot.sources")
    )
    items = tuple(
        _item(entry, "snapshot.items")
        for entry in _list(reader.field("items"), "snapshot.items")
    )
    findings = tuple(
        _group(entry, "snapshot.findings")
        for entry in _list(reader.field("findings"), "snapshot.findings")
    )
    notifications = tuple(
        _notification(entry, "snapshot.notifications")
        for entry in _list(reader.field("notifications"), "snapshot.notifications")
    )
    return SnapshotPayload(
        _enum(ScanState, reader.field("scanState"), "snapshot.scanState"),
        sources,
        _summary(reader.field("summary")),
        items,
        findings,
        notifications,
    )
...
def _inventory(value: JsonValue) -> InventoryPayload:
    ...
    items = tuple(
        ...
        for entry in _list(reader.field("items"), "inventory.items")
    )
    return InventoryPayload(
        _enum(ItemSource, reader.field("source"), "inventory.source"),
        _nonnegative_int(reader.field("total"), "inventory.total"),
        items,
    )
...
def _star_result(value: JsonValue) -> StarResultPayload:
    ...
    return StarResultPayload(
        ItemId(_identifier(reader.field("itemId"), "star-result.itemId")),
        _enum(WatchMode, reader.field("mode"), "star-result.mode"),
    )
...
def _source(value: JsonValue, path: str) -> SourceHealth:
    ...
    return SourceHealth(
        _enum(SourceName, reader.field("source"), f"{path}.source"),
        _enum(SourceStatus, reader.field("status"), f"{path}.status"),
        _enum(Provenance, reader.field("provenance"), f"{path}.provenance"),
        _timestamp(reader.field("observedAt"), f"{path}.observedAt"),
        _timestamp(reader.field("freshUntil"), f"{path}.freshUntil"),
        cause,
    )
...
def _summary(value: JsonValue) -> Summary:
    ...
    return Summary(
        _nonnegative_int(reader.field("totalUpdates"), "summary.totalUpdates"),
        _nonnegative_int(reader.field("watchedUpdates"), "summary.watchedUpdates"),
        _nonnegative_int(reader.field("securityFindings"), "summary.securityFindings"),
        _nonnegative_int(reader.field("degradedSources"), "summary.degradedSources"),
    )
...
def _item(value: JsonValue, path: str) -> NormalizedItem:
    ...
    return NormalizedItem(
        ItemId(_identifier(reader.field("id"), f"{path}.id")),
        _enum(ItemSource, reader.field("source"), f"{path}.source"),
        _identifier(reader.field("label"), f"{path}.label"),
        _optional_string(reader.field("installed"), f"{path}.installed"),
        _optional_string(reader.field("candidate"), f"{path}.candidate"),
        _enum(WatchMode, reader.field("watchMode"), f"{path}.watchMode"),
        _bool(reader.field("watchable"), f"{path}.watchable"),
        _enum(Provenance, reader.field("provenance"), f"{path}.provenance"),
    )
...
def _group(value: JsonValue, path: str) -> SecurityFindingGroup:
    ...
    return SecurityFindingGroup(
        ItemId(_identifier(reader.field("itemId"), f"{path}.itemId")),
        tuple(
            _finding(entry, f"{path}.findings")
            for entry in _list(reader.field("findings"), f"{path}.findings")
        ),
    )
...
def _finding(value: JsonValue, path: str) -> SecurityFinding:
    ...
    cve_ids = tuple(
        _identifier(entry, f"{path}.cveIds")
        for entry in _list(reader.field("cveIds"), f"{path}.cveIds")
    )
    return SecurityFinding(
        FindingId(_identifier(reader.field("id"), f"{path}.id")),
        ItemId(_identifier(reader.field("itemId"), f"{path}.itemId")),
        _identifier(reader.field("advisoryId"), f"{path}.advisoryId"),
        cve_ids,
        _enum(Severity, reader.field("severity"), f"{path}.severity"),
        _optional_string(reader.field("fixedVersion"), f"{path}.fixedVersion"),
        _bool(reader.field("knownExploited"), f"{path}.knownExploited"),
        _enum(Provenance, reader.field("provenance"), f"{path}.provenance"),
    )
...
def _notification(value: JsonValue, path: str) -> NotificationOutcome:
    ...
    return NotificationOutcome(
        NotificationFingerprint(
            _identifier(reader.field("fingerprint"), f"{path}.fingerprint")
        ),
        _enum(NotificationStatus, reader.field("status"), f"{path}.status"),
    )
...
def _error(value: JsonValue, path: str) -> ErrorInfo:
    ...
    return ErrorInfo(
        _enum(ErrorCode, reader.field("code"), f"{path}.code"),
        _bounded_string(
            reader.field("message"), f"{path}.message", MAX_ERROR_MESSAGE_LENGTH
        ),
    )
